      ...
      <header className="px-6 border-b border-black/[.08] bg-white relative z-20">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-3 lg:gap-8 text-sm">
            {["Tüm Ürünler","Erkek","Kadın","Çocuk","İndirim"].map((label) => (
              <div key={label} className="relative group">
                <a href={label === "Tüm Ürünler" ? "/products" : `/products?category=${encodeURIComponent(label.toLowerCase())}`} className={label === "İndirim" ? "text-red-600" : ""}>{label.toUpperCase()}</a>
                      ...
                      {Object.keys((catalog.categories || {})[label.toLowerCase()] || {}).map((grp) => (
                        <div key={grp}>
                          ...
                          <ul className="space-y-2 text-sm text-black/80">
                            <li>
                              <a href={`/products?category=${encodeURIComponent(`${label.toLowerCase()} ${grp.toLowerCase()}`)}`} className="hover:underline">{`${label} ${grp}`}</a>
                            </li>
                            {(((catalog.categories || {})[label.toLowerCase()] || {})[grp] || []).map((sub) => (
                              <li key={sub}>
                                <a href={`/products?category=${encodeURIComponent(`${label.toLowerCase()} ${grp.toLowerCase()} ${sub.toLowerCase()}`)}`} className="hover:underline">{sub}</a>
                              </li>
                            ))}
                          </ul>
                        </div>
          ...
          <div className="flex items-center gap-5 text-lg">
            ...
            <a href="/wishlist" title="İstek Listesi" aria-label="İstek Listesi" className="hover:opacity-80 relative">
              <i className="fa-regular fa-heart" />
            </a>
            <a href="/cart" title="Sepet" aria-label="Sepet" className="hover:opacity-80 relative">
            ...
            {!user && <a href="/login" className="md:hidden text-sm">Giriş</a>}
            {!user ? (
              <div className="relative group text-sm hidden md:block">
                ...
                <div className="pointer-events-none opacity-0 group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity duration-150 absolute right-0 top-full">
                  <div className="mt-3 w-56 bg-white shadow-xl border border-black/5 p-4 flex flex-col gap-2 text-sm">
                    <a href="/login" className="h-9 grid place-items-center" style={{ backgroundColor: "#223c6c", color: "#fff" }}>Giriş Yap</a>
                    <a href="/register" className="h-9 grid place-items-center" style={{ border: "1px solid #223c6c", color: "#223c6c" }}>Kayıt Ol</a>
                  </div>
                </div>
              </div>
            ) : (
              <div className="relative group text-sm">
                <a href="/profile" className="hidden md:flex items-center gap-2 hover:opacity-80">
        ...
        <div className={`md:hidden overflow-hidden transition-all duration-300 ${mobileOpen ? "max-h-[70vh] border-t border-black/10" : "max-h-0"}`}>
          <div className="px-6 py-4 space-y-4 bg-white">
            <a href="/products" className="block text-sm font-semibold">TÜM ÜRÜNLER</a>
                  ...
                  <div className="overflow-hidden">
                    {Object.keys((catalog.categories||{})[gender]||{}).map((grp)=> (
                      <div key={grp} className="py-2">
                        <a href={`/products?category=${encodeURIComponent(`${gender} ${grp}`)}`} className="block text-sm py-1">{grp.charAt(0).toUpperCase()+grp.slice(1)}</a>
                        <div className="pl-3 space-y-1">
                          {(((catalog.categories||{})[gender]||{})[grp]||[]).map((sub)=> (
                            <a key={sub} href={`/products?category=${encodeURIComponent(`${gender} ${grp} ${sub}`)}`} className="block text-xs text-black/70">{sub}</a>
                          ))}
                        </div>
                      </div>
                    ))}
            ...
            <div className="pt-2 grid grid-cols-2 gap-2">
              {!user && (
                <>
                  <a href="/login" className="text-center rounded-xl bg-[#223c6c] text-white py-2 text-sm">Giriş Yap</a>
                  <a href="/register" className="text-center rounded-xl border border-[#223c6c] text-[#223c6c] py-2 text-sm">Kayıt Ol</a>
                </>
              )}
              {user && (
                <>
                  <a href="/profile" className="text-center rounded-xl border border-[#223c6c] text-[#223c6c] py-2 text-sm">Hesabım</a>
